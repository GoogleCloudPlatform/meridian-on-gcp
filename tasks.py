...
@task(pre=[setup_uv_config])
def fix(c):  # noqa: ANN001, ANN201
    """Apply linting fixes"""
    c.run("uv run black *.py **/*.py --force-exclude .venv")
    c.run("uv run isort --profile google *.py **/*.py")


@task(pre=[setup_uv_test])
def test(c):  # noqa: ANN001, ANN201
    """Run unit tests"""
    c.run("uv run pytest python/")
